...
def get_average_attn_map(tokenizer, prompt, ca_token, max_height=256, max_width=256):  
    averaged_map = None
    ...
    for timestep in attn_maps.keys():
        ...
        for path in attn_maps[timestep].keys():
            ...
            timestep_map = timestep_map + value if timestep_map is not None else value

        averaged_map = averaged_map + timestep_map if averaged_map is not None else timestep_map

    averaged_map = averaged_map / len(attn_maps)

    # Match with `ca` token
    tokens = prompt2tokens(tokenizer, prompt)
    ca_token += '</w>'
    token_index = tokens.index(ca_token)
    token_attn_map = averaged_map[token_index]

    # Min-max normalization (keeping it as float32)
    min_val = token_attn_map.min()
    max_val = token_attn_map.max()
    normalized_token_attn_map = (token_attn_map - min_val) / (max_val - min_val)

    # The tensor is returned as float32 with gradients intact
    return normalized_token_attn_map